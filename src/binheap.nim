...
proc newBinaryHeapOfCap*[O: static[bool], T: Comparable](capacity: Positive): BinaryHeap[O, T] =
...
proc `=destroy`[O: static[bool], T: Comparable](x: var BinaryHeap[O, T]) =
  if likely(not x.raw.isNil):
    dealloc x.raw

proc `=copy`[O: static[bool], T: Comparable](x: var BinaryHeap[O, T], y: BinaryHeap[O, T]) =
...
proc heapify[O: static[bool], T: Comparable](heap: var BinaryHeap[O, T], start: Natural) =
  let
    left  = 2 * start + 1
    right = 2 * start + 2
...
proc insert*[O: static[bool], T: Comparable](heap: var BinaryHeap[O, T], newItem: T) =
  var
    idx = block:
      var res = 0
...
proc swapHead*[O: static[bool], T: Comparable](heap: var BinaryHeap[O, T], newHead: T): T =
  result = heap.raw[0]
  heap.raw[0] = newHead
  heapify(heap, 0)

proc popHead*[O: static[bool], T: Comparable](heap: var BinaryHeap[O, T]): T =
  when O: # MaxHeap
    heap.swapHead(low(T))
  else: # MinHeap
